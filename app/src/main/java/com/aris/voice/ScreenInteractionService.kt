...
class ScreenInteractionService : AccessibilityService() {
    ...
    companion object {
        ...
        val DEBUG_SHOW_TAPS = BuildConfig.DEBUG

        val DEBUG_SHOW_BOUNDING_BOXES = BuildConfig.DEBUG
    }
    ...
    private var isVolumeDownDown = false
    private var isPowerDown = false
    private var lastVolumeDownTime = 0L

    private fun getAppNameFromPackage(packageName: String?): String {
        if (packageName.isNullOrBlank()) return "Phone"
        if (packageName == this.packageName) return "Aris App"
        return try {
    ...
    private fun logActivityMoment(actionDescription: String) {
        val now = System.currentTimeMillis()
        if (actionDescription == lastLoggedAction && now - lastLogTime < 5000L) {
            return
        }
        lastLoggedAction = actionDescription
        lastLogTime = now
        ...
        serviceScope.launch {
            try {
                ...
                val momentEntry = hashMapOf(
                    "task" to actionDescription,
                    "status" to "completed",
                ...
                db.collection("users").document(currentUser.uid)
                    .update("taskHistory", FieldValue.arrayUnion(momentEntry))
                Log.d("ScreenInteractionService", "Captured Activity Moment logged: $actionDescription")
            } catch (e: Exception) {
                Log.e("ScreenInteractionService", "Error logging capture moment: ${e.message}")
            }
        }
    }
...
//        setupWaveBorderEffect()
    ...
    override fun onKeyEvent(event: android.view.KeyEvent): Boolean {
        ...
        if (keyCode == android.view.KeyEvent.KEYCODE_VOLUME_DOWN) {
            if (action == android.view.KeyEvent.ACTION_DOWN) {
                isVolumeDownDown = true
                val now = System.currentTimeMillis()
                
                // Trigger conditions:
                // 1. Both Volume Down and Power key are held
                // 2. OR Volume Down is pressed twice rapidly (within 500ms) as a reliable fallback
                if (isPowerDown || (now - lastVolumeDownTime < 500L)) {
                    triggerAgentWakeUp()
    ...
    private fun triggerAgentWakeUp() {
        Log.d("ScreenInteractionService", "Physical shortcut triggered! Initiating ConversationalAgentService.")
    ...
    private fun drawDebugBoundingBoxes(elements: List<SimplifiedElement>) {
        ...
        if (statusBarHeight < 0) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                ...
                val insets = windowMetrics.windowInsets.getInsetsIgnoringVisibility(android.view.WindowInsets.Type.statusBars())
        ...
        val windowManager = this.windowManager ?: getSystemService(Context.WINDOW_SERVICE) as WindowManager
...
//        return filteredElements.sortedWith(compareBy({ it.bounds.top }, { it.bounds.left }))
    ...
    private fun deduplicateElements(elements: List<SimplifiedElement>): List<SimplifiedElement> {
        val result = mutableListOf<SimplifiedElement>()
        val seenDescriptions = HashSet<String>()
        
        elements.sortedByDescending { it.bounds.width() * it.bounds.height() }
            .forEach { element ->
                if (element.description !in seenDescriptions) {
                    seenDescriptions.add(element.description)
                    result.add(element)
                }
            }
        return result
    ...
    private fun extractSimplifiedElementsDirect(rootNode: AccessibilityNodeInfo): List<SimplifiedElement> {
        val elements = mutableListOf<SimplifiedElement>()
        traverseNodeDirect(rootNode, elements)
        return deduplicateElements(elements)
    }

    private fun traverseNodeDirect(node: AccessibilityNodeInfo?, elements: MutableList<SimplifiedElement>) {
        if (node == null) return
        ...
        val className = node.className?.toString() ?: "Element"

        if (isClickable || text.isNotEmpty() || (contentDesc.isNotEmpty() && contentDesc != "null")) {
            val description = when {
            ...
            if (description.isNotEmpty()) {
                ...
                if (bounds.width() > 0 && bounds.height() > 0) {
                    val center = Point(bounds.centerX(), bounds.centerY())
                    elements.add(SimplifiedElement(description, bounds, center, isClickable, className))
                }
            }
        }
        ...
        for (i in 0 until node.childCount) {
            ...
            if (child != null) {
                traverseNodeDirect(child, elements)
                child.recycle()
            }
        }
    }
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent) {
        ...
        if (eventType == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED) {
            ...
            if (!packageName.isNullOrBlank() && !className.isNullOrBlank()) {
                ...
                // Track real-time activity moments
                if (packageName != this.packageName && 
                    packageName != "com.android.systemui" && 
                    !packageName.contains("launcher") && 
                    !packageName.contains("inputmethod")) {
                    val appName = getAppNameFromPackage(packageName)
                    logActivityMoment("Opened $appName")
                }
            }
            ...
            if (!packageName.isNullOrBlank() && 
                ...
                val textStr = textList.joinToString(" ").trim()
                val contentDesc = event.contentDescription?.toString()?.trim() ?: ""
                val elementText = if (textStr.isNotEmpty()) textStr else contentDesc
                
                if (elementText.isNotEmpty() && elementText.length < 100) {
                    logActivityMoment("Clicked '$elementText' in $appName")
                }
            }
            ...
            if (!packageName.isNullOrBlank() && 
                ...
                val appName = getAppNameFromPackage(packageName)
                val textList = event.text ?: emptyList()
                val typedText = textList.joinToString(" ").trim()
                
                if (typedText.isNotEmpty() && typedText.length < 80) {
                    logActivityMoment("Typed '$typedText' in $appName")
                }
            }
        }
    }
    ...
    /**
     * Taps an element by text.
     */
    fun tapElementByText(text: String): Boolean {
        ...
        // Fallback to finding bounds and clicking coordinates
        val allElements = extractSimplifiedElementsDirect(rootNode)
        val element = allElements.find { it.description.contains(text, ignoreCase = true) }
        if (element != null) {
            clickOnPoint(element.center.x.toFloat(), element.center.y.toFloat())
            return true
        }
        return false
    }
    ...
    fun clickOnPoint(x: Float, y: Float) {
        val floatingService = com.aris.voice.services.FloatingArisButtonService.instance
        if (floatingService != null) {
            floatingService.animateToPointAndTap(x, y) {
                if (DEBUG_SHOW_TAPS) {
                    showDebugTap(x, y)
                }
    ...
    fun typeTextInFocusedField(textToType: String) {
        ...
        if (focusedNode != null && focusedNode.isEditable) {
            ...
            val newText = existingText + textToType
    ...
    fun performEnter() {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.R) {
            Log.w("InteractionService", "performEnter requires API 30+.")
            return
        }
