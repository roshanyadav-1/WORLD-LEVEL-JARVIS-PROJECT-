...
import com.aris.voice.utilities.UserIdManager
import com.aris.voice.utilities.UserProfileManager
import com.aris.voice.utilities.VideoAssetManager
...
import com.aris.voice.utilities.DeltaStateColorMapper
import com.aris.voice.views.DeltaSymbolView
...
class MainActivity : BaseNavigationActivity() {
    ...
    private lateinit var stateChangeListener: (ArisState) -> Unit
    ...
    private lateinit var arisCreature: ArisCreatureView
    ...
    companion object {
        ...
        const val ACTION_PURCHASE_UPDATED = "com.aris.voice.PURCHASE_UPDATED"
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<View>(R.id.btn_set_default_assistant).setOnClickListener {
        ...
        arisCreature = findViewById(R.id.aris_creature)
        freemiumManager = FreemiumManager()
        updateStatusText(ArisState.IDLE)
        setupProBanner()
        initializeArisStateManager()
    ...
    private fun handleIntent(intent: Intent?) {
        if (intent?.action == "com.aris.voice.WAKE_UP_ARIS") {
            Logger.d("MainActivity", "Wake up Axel shortcut activated!")
            startConversationalAgent()
        } else if (intent?.action == "com.aris.voice.ACTION_WIDGET_TAP") {
            Logger.d("MainActivity", "Widget tap activated! Driving Force Listen mode.")
            startConversationalAgentForceListen()
        }
    }
    ...
    private fun startServiceSafely(intent: Intent) {
        try {
            ...
            Logger.e("MainActivity", "Failed to start foreground service safely: ${e.message}", e)
        }
    }

    private fun startConversationalAgentForceListen() {
        val intent = Intent(this, com.aris.voice.ConversationalAgentService::class.java).apply {
    ...
    private fun startConversationalAgent() {
        if (!ConversationalAgentService.isRunning) {
            ...
            Toast.makeText(this, "A.R.I.S is waking up...", Toast.LENGTH_SHORT).show()
        } else {
            Logger.d("MainActivity", "ConversationalAgentService is already running.")
            Toast.makeText(this, "A.R.I.S is already awake!", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun setupClickListeners() {
        findViewById<View>(R.id.btn_diagnostics_center).setOnClickListener {
        ...
        // Add click listener to arisCreature and center_content
        val centerContent = findViewById<View>(R.id.center_content)
        val wakeAction = View.OnClickListener {
            if (arisStateManager.getCurrentState() == ArisState.IDLE || arisStateManager.getCurrentState() == ArisState.ERROR) {
                startConversationalAgent()
            }
        }
        
        arisCreature.setOnClickListener(wakeAction)
        
        // Add single-tap, double-tap, and long-press support for arisCreature
        ...
        val longPressRunnable = Runnable {
            ...
            // Long Press Triggered: start listening!
            val intent = Intent(this, com.aris.voice.ConversationalAgentService::class.java).apply {
                action = com.aris.voice.ConversationalAgentService.ACTION_FORCE_LISTEN
            }
            startServiceSafely(intent)
        ...
        arisCreature.setOnTouchListener { view, event ->
            when (event.action) {
                ...
                android.view.MotionEvent.ACTION_UP -> {
                    ...
                    if (!isLongPressTriggered) {
                        ...
                        if (upTime - lastUpTime < 350) {
                            // Double tap! Stop completely and spin stars
                            val intent = Intent(this, com.aris.voice.ConversationalAgentService::class.java).apply {
                                action = com.aris.voice.ConversationalAgentService.ACTION_TRIGGER_DIZZY_SHUTDOWN
                            }
                            startServiceSafely(intent)
                        } else {
        ...
        centerContent.setOnClickListener(wakeAction)
        deltaSymbol.setOnClickListener(wakeAction)
        findViewById<View>(R.id.btn_wake_agent)?.setOnClickListener(wakeAction)
    }
    ...
    private fun requestLimitIncrease() {
        ...
        val recipient = "roshanyadavofficial4@gmail.com"
    ...
    /**
     * Initialize ArisStateManager and set up state change listeners
     */
    private fun initializeArisStateManager() {
        ...
        stateChangeListener = { newState ->
            ...
            Logger.d("MainActivity", "Aris state changed to: ${newState.name}")
        }
        arisStateManager.addStateChangeListener(stateChangeListener)
    }
    private fun updateDeltaVisuals(state: ArisState) {
        runOnUiThread {
            ...
            // Set live animated creature state
            arisCreature.setState(state)
        }
    }
    ...
    override fun onResume() {
        ...
        updateDeltaVisuals(arisStateManager.getCurrentState())
        updateUI()
        arisStateManager.startMonitoring()
        
        // Auto-start the cute floating Axel widget over other apps if permission is present
        if (Settings.canDrawOverlays(this)) {
            val serviceIntent = Intent(this, com.aris.voice.services.FloatingArisButtonService::class.java)
            startServiceSafely(serviceIntent)
        }
    ...
    override fun onDestroy() {
        super.onDestroy()
        if (::arisStateManager.isInitialized && ::stateChangeListener.isInitialized) {
    ...
    private fun showDisclaimerDialog() {
        val dialog = AlertDialog.Builder(this)
            .setTitle("Disclaimer")
            .setMessage("A.R.I.S is an experimental AI assistant and is still in development. It may not always be accurate or perform as expected. It does small task better. Your understanding is appreciated!")
    ...
    private fun updateUI() {
        ...
        if (allPermissionsGranted) {
            ...
            managePermissionsButton.visibility = View.VISIBLE
    ...
    private fun updateDefaultAssistantButtonVisibility() {
        val btn = findViewById<View>(R.id.btn_set_default_assistant)
...
//                }
    ...
    /**
     * Update the status text based on the current ArisState
     */
    fun updateStatusText(state: ArisState) {
        runOnUiThread {
            try {
