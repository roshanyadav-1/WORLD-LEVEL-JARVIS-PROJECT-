        ...
        fun bind(trigger: Trigger) {
            ...
            when (trigger.type) {
                ...
                TriggerType.CHARGING_STATE -> {
                    timeTextView.text = "Battery Status Trigger"
                }
                TriggerType.APP_OPENED -> {
                    timeTextView.text = "App opened: ${trigger.targetAppName}"
                }
                TriggerType.LOCATION_BASED -> {
                    timeTextView.text = "Location: lat=${trigger.locationLatitude}"
                }
                TriggerType.BATTERY_LEVEL -> {
                    timeTextView.text = "Battery level trigger"
                }
                TriggerType.CALENDAR_EVENT -> {
                    timeTextView.text = "Calendar event: ${trigger.calendarEventTitle}"
                }
                TriggerType.CONTACT_CALL -> {
                    timeTextView.text = "Contact call: ${trigger.contactNameOrNumber}"
                }
                TriggerType.SCREEN_ON_OFF -> {
                    timeTextView.text = "Screen trigger"
                }
                TriggerType.COMPOUND -> {
                    timeTextView.text = "Compound trigger"
                }
            }
