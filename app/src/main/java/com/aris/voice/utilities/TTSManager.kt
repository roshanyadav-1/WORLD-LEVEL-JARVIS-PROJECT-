...
class TTSManager private constructor(private val context: Context) : TextToSpeech.OnInitListener {
        ...
        if (status == TextToSpeech.SUCCESS) {
            nativeTts?.setOnUtteranceProgressListener(object : UtteranceProgressListener() {
                ...
                override fun onError(utteranceId: String?) {
                    utteranceListener?.invoke(false)
                    nativeTtsDeferred?.completeExceptionally(Exception("Native TTS error: $utteranceId"))
                }
            })
    ...
    fun stop() {
        ...
        try {
            ...
            Log.e("TTSManager", "Error stopping native voice during stop()", e)
        }
        nativeTtsDeferred?.complete(Unit)
    }
    ...
    private suspend fun speak(text: String) {
        try {
            // Smart chunking: Break text into sentences of ~50 words each so captions align perfectly with speech
            val textChunks = chunkTextIntoSentences(text, maxWordsPerChunk = 50)

            for (chunk in textChunks) {
                val trimmed = chunk.trim()
                if (trimmed.isNotEmpty()) {
                    speakNativeChunk(trimmed)
                }
            }
        } catch (e: Exception) {
    ...
    private suspend fun speakNativeChunk(chunk: String) {
        try {
            isNativeTtsInitialized.await()
            val nativeDeferred = CompletableDeferred<Unit>()
            this.nativeTtsDeferred = nativeDeferred
            ...
            val utteranceId = "native_${System.currentTimeMillis()}"
            val result = nativeTts?.speak(chunk, TextToSpeech.QUEUE_FLUSH, null, utteranceId)

            if (result == TextToSpeech.SUCCESS) {
                // Wait for the native speech to complete, estimating standard timeout based on string log
                val calculatedTimeout = (chunk.length * 100L) + 5000L
                withTimeoutOrNull(calculatedTimeout) {
                    nativeDeferred.await()
                }
            } else {
                Log.e("TTSManager", "Native speak initiation failed directly.")
            }
            ...
            Log.e("TTSManager", "Failed to speak native chunk: ${e.message}")
        }
    }
