    ...
    private fun startWakeWordService() {
        if (ContextCompat.checkSelfPermission(context, Manifest.permission.RECORD_AUDIO) == PackageManager.PERMISSION_GRANTED) {
            ...
            try {
                ...
                android.util.Log.e("WakeWordManager", "Failed to start EnhancedWakeWordService: ${e.message}", e)
                Toast.makeText(context, "Failed to start wake word services", Toast.LENGTH_SHORT).show()
            }
        } else {
