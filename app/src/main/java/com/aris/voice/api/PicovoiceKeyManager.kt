...
class PicovoiceKeyManager(private val context: Context) {
    ...
    private suspend fun fetchAccessKeyFromApi(): String? = withContext(Dispatchers.IO) {
        try {
            ...
            if (!isOnline) {
                ...
                try { com.aris.voice.utilities.NetworkNotifier.notifyOffline() } catch (e: Exception) {}
                return@withContext null
            }
    ...
    /**
     * Gets the cached access key from SharedPreferences if not expired (expires after 7 days)
     */
    private fun getCachedAccessKey(): String? {
        val key = sharedPreferences.getString(KEY_ACCESS_KEY, null) ?: return null
        val timestamp = sharedPreferences.getLong("access_key_timestamp", 0L)
        val currentTime = System.currentTimeMillis()
        val sevenDaysMs = java.util.concurrent.TimeUnit.DAYS.toMillis(7)
        if (currentTime - timestamp > sevenDaysMs) {
            Log.d(TAG, "Cached Picovoice key expired after 7 days, forcing refresh.")
            return null
        }
        return key
    }
    ...
    /**
     * Saves the access key to SharedPreferences with current timestamp
     */
    private fun saveAccessKeyToCache(accessKey: String) {
        sharedPreferences.edit {
            putString(KEY_ACCESS_KEY, accessKey)
            putLong("access_key_timestamp", System.currentTimeMillis())
        }
    }
